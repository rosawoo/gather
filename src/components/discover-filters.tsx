...
export function DiscoverFilters({ neighborhoods }: Options) {
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <input
            ...
            className="w-full rounded-full border border-neutral-200 bg-white px-4 py-2.5 pl-9 text-sm text-gather-ink outline-none transition placeholder:text-neutral-400 focus:border-gather-accent focus:ring-2 focus:ring-gather-accent/40"
          />
          <svg
            aria-hidden
            viewBox="0 0 24 24"
      ...
      {activeCount > 0 && !open ? (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <button
            type="button"
            onClick={clearAll}
            className="text-[11px] font-semibold uppercase tracking-[0.14em] text-neutral-500 transition hover:text-gather-ink"
          >
            Clear all
          </button>
        </div>
...
function Chip({ label, onClear }: { label: string; onClear: () => void }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-gather-brown/[0.08] px-2.5 py-1 text-xs font-medium text-gather-brown">
      {label}
      <button
        type="button"
        onClick={onClear}
        className="text-gather-brown/60 transition hover:text-gather-brown"
